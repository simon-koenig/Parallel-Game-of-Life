                            ...
                            int mpi_numproc, int ndims)
{
  ...
  {
    std::cout << "destinations" << destinations[i];
  ...
  for (int iter = 0; iter <= iterations; ++iter)
  {
    ...
    // int sendbuf[4];
    // int recvbuf[4];
    // sendbuf[0] = NORTH_SEND.data();
    // sendbuf[1] = SOUTH_SEND.data();
    // sendbuf[2] = EAST_SEND.data();
    // sendbuf[3] = WEST_SEND.data();

    // int sendcounts[4] = {1, 1, 1, 1};
    // int recvcounts[4] = {1, 1, 1, 1};
    // MPI_Aint send_displs[4] = {0 * sizeof(int), 1 * sizeof(int), 2 * sizeof(int), 3 * sizeof(int)};
    // MPI_Aint recv_displs[4] = {0 * sizeof(int), 1 * sizeof(int), 2 * sizeof(int), 3 * sizeof(int)};
    // MPI_Datatype send_types[4] = {MPI_INT, MPI_INT, MPI_INT, MPI_INT};
    // MPI_Datatype recv_types[4] = {MPI_INT, MPI_INT, MPI_INT, MPI_INT};

    // // Create neighborhood communication
    // MPI_Comm graph_comm;
    // MPI_Dist_graph_create_adjacent(cart_comm, 4, rank_neighbors, MPI_UNWEIGHTED, 4, rank_neighbors, MPI_UNWEIGHTED, MPI_INFO_NULL, 0, &graph_comm);
    // MPI_Neighbor_alltoallw(sendbuf, sendcounts, send_displs, send_types, recvbuf, recvcounts, recv_displs, recv_types, graph_comm);
    ...
    // Print neighbors in new communicator
    // std::cout << "******** My rank: " << myrank << " ********* \n";
    // std::cout << "******** neighbors: " << myrank << " ********* \n";

    for (int i = 0; i < data_size; ++i)
    {
      std::cout << "Rank " << myrank << " send: size =" << sendBuffer.size()
                << ", data =";
      if (i < 4)
      {
        for (int j = 0; j < NX; ++j)
        {
          std::cout << sendBuffer[i * NX + j] << " ";
        }
      }
      else
      {
        std::cout << sendBuffer[4 * NX + i - 4] << " ";
      }
      std::cout << std::endl;
      fflush(stdout);
    }
    ...
    //
    // MPI_Barrier(GRAPH_COMM); // Barrier before all send and recv
    //  *************** Perform send *************** //
    ...
    for (int i = 0; i < data_size; ++i)
    {
      std::cout << "Rank " << myrank << " recv: size =" << recvBuffer.size()
                << ", data =";
      if (i < 4)
      {
        for (int j = 0; j < NX; ++j)
        {
          std::cout << recvBuffer[i * (NX) + j] << " ";
        }
      }
      else
      {
        std::cout << recvBuffer[4 * (NX) + i - 4] << " ";
      }
      std::cout << std::endl;
      fflush(stdout);
    }
    ...
    // Assuming recvBuffer is correctly filled

    // Send and receive buffers have to be accesed inversely from
    // recvBuffer than they were written to the send buffer
